...
class SEDResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # x shape: (batch, 1, 80, T)
        # ResNet은 (batch, channel, H, W) 형태를 기대
...
class ASTModel(nn.Module):
    ...
    def __init__(self, num_classes=2, input_channels=1, hidden_dim=256):
        ...
        self.feature_extractor = nn.Sequential(
            ...
            nn.MaxPool2d(2, 2),  # (80, T) -> (40, T/2)
            
            # Conv Block 2
            nn.Conv2d(64, 128, kernel_size=3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, 2),  # (40, T/2) -> (20, T/4)
            
            # Conv Block 3
            nn.Conv2d(128, 256, kernel_size=3, padding=1),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, 2),  # (20, T/4) -> (10, T/8)
...
if __name__ == "__main__":
    ...
    # 더미 입력 (batch=2, channel=1, mel_bins=80, time_frames=150)
